...
class VLASSCat:
    ...
    def _load(self):
        ...
        except:   # try CIRADA columns
            vlass.drop(columns=['Component_name', 'Component_id', 'Isl_id', 'E_RA', 'E_DEC', 'E_Total_flux', 'Peak_flux', 'E_Peak_flux', 'Maj', 'E_Maj', 'Min', 'E_Min', 'PA', 'E_PA', 'Isl_Total_flux', 'E_Isl_Total_flux', 'Isl_rms', 'Isl_mean', 'Resid_Isl_rms', 'Resid_Isl_mean', 'RA_max', 'DEC_max', 'E_RA_max', 'E_DEC_max', 'Xposn', 'E_Xposn', 'Yposn', 'E_Yposn', 'Xposn_max', 'E_Xposn_max', 'Yposn_max', 'E_Yposn_max', 'Maj_img_plane', 'E_Maj_img_plane', 'Min_img_plane', 'E_Min_img_plane', 'PA_img_plane', 'E_PA_img_plane', 'DC_Maj', 'E_DC_Maj', 'DC_Min', 'E_DC_Min', 'DC_PA', 'E_DC_PA', 'DC_Maj_img_plane', 'E_DC_Maj_img_plane', 'DC_Min_img_plane', 'E_DC_Min_img_plane', 'DC_PA_img_plane', 'E_DC_PA_img_plane', 'Tile', 'Subtile', 'QL_image_RA', 'QL_image_DEC', 'NVSS_distance', 'FIRST_distance', 'Peak_to_ring', 'Duplicate_flag', 'Quality_flag', 'Source_name', 'Source_type', 'QL_cutout', 'NN_dist', 'BMAJ', 'BMIN', 'BPA', 'Best_neuron_y', 'Best_neuron_x', 'Neuron_dist', 'P_sidelobe'], inplace=True)
    ...
    def skycoords(self) -> SkyCoord:
        ras = self._catalog["RA"].tolist()
        decs = self._catalog["DEC"].tolist()
...
def make_cat(filename, path='/Users/claw/data/dsa110/'):
    df = pandas.read_csv(f'{os.path.join(path, filename)}')
    ras = df["RA"].tolist()
    decs = df["DEC"].tolist()

    cat = SkyCoord(ras, decs, unit='deg')

    print(f"Read catalog of {len(cat)} sources")
    return cat


def match_ps1(dsacat, ndet=2, radius=5/3600):
    """ Takes astropy catalog (SkyCoord with multiple entries) and matches each to PS1 catalog.
    Returns subset of catalog with matches, tuple of delta RA, and tuple of delta Dec. 
    ndet is number of PS1 detections (epochs) and radius is PS1 search radius in deg.
    Only returns unique PS1 matches within radius with ndet detections.
    """
